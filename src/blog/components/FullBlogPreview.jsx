...
`;
export default function FullBlogPreview(props) {
  const [blogMarkdown, setBlogMarkdown] = useState("Markdown is empty...");
  
  
  useEffect(function(){
    import(`../../assets/markdown/blog/${props.data["markdownLocal"]}`)
    .then(res => {
        fetch(res.default)
            .then(res => res.text())
  ...
  return (
    <article className="blog-post mb-4">
      ...
      {/* <p className="blog-post-meta">{data["dateCreated"]} by <Link to={data["author"]}>{data["author"]}</Link></p> */}
